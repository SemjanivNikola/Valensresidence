                ...
                <div class="calendar-col col-lg-6 col-display">
                  <table class="w-100">
                    <tr style="height: 80px">
                      ...
                      >
                        ...
                        >
                          {{ currentYear }}<br /><b
                            v-if="$store.getters.getLang === 'eng'"
                            >{{ currentMonth.fullNameEng }}</b
                          ><b v-else>{{ currentMonth.fullNameHr }}</b>
                        </h4>
                      </th>
                      <td colspan="2" style="width: 28%">
                        <button @click="prevGo()" class="cal-btn">
                          <b-icon-chev-left></b-icon-chev-left>
                        </button>
                        <button @click="nextGo()" class="cal-btn">
                          <b-icon-chev-right></b-icon-chev-right>
                        </button>
                      </td>
                    </tr>
                  </table>
                  <table class="w-100">
                    ...
                    <tr id="row-days" style="border-bottom: 2px solid #cecece">
                      <td
                        class="last-child-row"
                        v-for="d in days"
                        v-bind:key="d.num"
                      >
                        <p v-if="$store.getters.getLang === 'eng'" class="code">
                          {{ d.nameEng }}
                        </p>
                        <p v-else class="code">{{ d.nameHr }}</p>
                      </td>
                    </tr>
                    ...
                    <tr
                      v-else
                      v-for="(week, wIndex) in dateList[currentMonth.num - 1]
                        .weeks"
                      v-bind:key="wIndex"
                      style="height: 50px"
                    >
                      <td
                        v-bind:class="[day.class, 'last-child-row']"
                        v-for="day in week"
                        v-bind:key="day.id"
                        @click="clickEvents(day, wIndex, currentMonth)"
                      >
                          ...
                          <div class="column-inner">
                            <div class="wrapper">
                              ...
                              <transition name="fadeInRight" mode="out-in">
                                <h5 v-if="selectedDays.length">
                                  {{ selectedDays[0].value }}.
                                  {{ arrivalMonth.name }} {{ arrivalYear }}
                                </h5>
                              </transition>
                            </div>
                          </div>
                        ...
                        <div class="col-sm-6 col-display">
                          <div class="column-inner">
                            <div class="wrapper">
                              <p>To:</p>
                              <transition name="fadeInRight" mode="out-in">
                                <h5 v-if="selectedDays.length === 1">
                                  {{ selectedDays[0].value }}.
                                  {{ arrivalMonth.name }} {{ arrivalYear }}
                                </h5>
                                <h5
                                  v-if="selectedDays.length > 1"
                                  :key="
                                    selectedDays[selectedDays.length - 1].value
                                  "
                                >
                                  {{
                                    selectedDays[selectedDays.length - 1].value
                            ...
                            <div class="wrapper">
                              <transition name="fadeInRight">
                                <div
                                  ...
                                  class="goto-btn text-center h-align"
                                >
                                  <div
                                    class="btn h-align"
                                    style="width: 245px"
                                    @click="eraseSelectedDays('all')"
                                  >
                                    <span
                                      style="
                                        font-size: 18px;
                ...
                <div class="queryForm-col col-lg-6 col-display">
                  <div class="text-left">
                    <div class="inquiry-headline-container">
                      <h2 class="blue-color" style="margin-bottom: 50px">
                        {{ webData.inquiry.title }}
                      </h2>
                      <div>
                        <form class="inquiry-form">
                          ...
                          <label class="block" for="phone">{{
                            webData.inquiry.phone
                          }}</label>
                          <input
                            class="block"
                            type="number"
                            id="phone"
                            v-model="inquiryForm.phoneNumber"
                            required
                          />
                          <label class="block" for="subject">{{
                            webData.inquiry.subject
                          }}</label>
                          <input
                            class="block"
                            type="text"
                          ...
                          <textarea
                            ...
                            rows="5"
                            cols="60"
                            v-model="inquiryForm.details"
                            required
                            :placeholder="webData.inquiry.details"
                          ></textarea>
                        ...
                        <div class="form-btn-container">
                          ...
                          <button v-else @click="submitForm" class="form-btn">
...
export default {
  ...
  watch: {
    selectService (status) {
      ...
      if (status === 1) { this.serviceDetailsOptions = this.webData.inquiry.persons } else if (status === 2) { this.serviceDetailsOptions = this.webData.inquiry.events }
    },
    $route () {
      this.resetForm()
    }
  ...
  async created () {
    ...
    const data = this.$store.getters.getBookingData
    ...
    if (this.dateList !== undefined && this.dateList.length > 0) { this.loading = false } else {
      this.daysInFeb()
  ...
  methods: {
    ...
    async saveCalendarData () {
      await this.$store.commit('setCalendarData', {
    ...
    async retrieveCalendarData () {
      ...
      const i = await this.$store.getters.getCalendarData
    ...
    async getBookedDates () {
      const id = '647379'
      await axios
        .get('https://h3.solutions/api/reservations/' + id)
        .then((response) => {
    ...
    async backupApiReservations (id) {
      await axios
        .get('https://www.h3.solutions/b-api/api/reservations/' + id)
        .then((response) => {
          this.handleAxiosDataResponse(response)
        })
        .catch((error) => {
    ...
    handleAxiosDataResponse (r) {
      ...
      const t2 = t1.filter((item) => item.year === this.currentYear)
    ...
    setUpCalendar: function () {
      ...
      this.months.forEach(function (value) {
        startingDay =
          Number(new Date(this.currentYear, value.num - 1).getDay()) - 1
    ...
    setUpTodaysDate: function (day, month, dayNum, year) {
      ...
      const t = this.months.filter((m) => m.name === month)

      this.today.month = this.currentMonth = t[0]
    },
    async setToday () {
      if (this.currentYear === this.today.year) {
        const m = await this.dateList.filter(
          (item) => Number(item.month) === this.today.month.num
        )
        m[0].weeks.forEach(function (item) {
          const i = item.filter(
            (itemInner) => itemInner.value === this.today.day
          )
          if (i.length > 0) {
    ...
    async prevGo () {
      if (
        ...
        this.currentMonth.num > this.today.month.num
      ) { this.currentMonth = this.months[this.currentMonth.num - 2] } else if (this.currentYear > this.today.year) {
    ...
    async nextGo () {
      if (this.currentMonth.num < 12) {
        ...
        if (
          !this.isObjectEmpty(this.tempSave) &&
          this.currentYear === this.tempSave.arrivalYear
        ) { this.handleSelectedDates() } else if (temp-- === this.arrivalYear) {
    ...
    clickEventMidHandler (day, month) {
      const objState = this.isObjectEmpty(this.tempSave)
      if (objState) {
        if (this.currentYear > this.today.year) return true
        else if (
          this.currentYear === this.today.year &&
          month.num > this.today.month.num
        ) { return true } else {
          return (
            this.currentYear === this.today.year &&
            month.num === this.today.month.num &&
            day.value > this.today.day
          )
        }
      } else {
    ...
    clickEvents (day, weekIndex, month) {
      if (this.clickEventMidHandler(day, month)) {
        ...
        if (this.arrivalDay === '' && day.class === 'free') {
          ...
          day.value === this.arrivalDay.value &&
          month.num === this.arrivalMonth.num
        ) {
          ...
          day.value === this.departureDay.value &&
          month.num === this.departureMonth.num
        ) {
    ...
    bookingOptions () {
      if (this.arrivalMonth.num === this.departureMonth.num) {
        const month = this.dateList[this.arrivalMonth.num - 1]
        this.selectingDaysThroughLoop(month.weeks, 1)
      } else {
        const months = []
        this.dateList.forEach(function (m) {
          if (
            m.month === this.arrivalMonth.num ||
            (m.month > this.arrivalMonth.num &&
              m.month <= this.departureMonth.num)
          ) {
            months.push(m)
          }
        }, this)
        ...
        months.forEach(function (m, index) {
          if (!breakPoint) {
            ...
            } else if (index === length - 1) { breakPoint = this.selectingDaysThroughLoop(m.weeks, 4) } else breakPoint = this.selectingDaysThroughLoop(m.weeks, 3)
          }
        }, this)
      }
      this.$store.commit('setBookingData', {
        ...
        arrivalMonth: this.arrivalMonth,
        departureYear: this.departureYear,
        departureMonth: this.departureMonth,
    ...
    selectingDaysThroughLoop (weeks, selector) {
      ...
      for (let i = this.startWeekIndex; i < weeks.length; i++) {
        weeks[i].forEach(function (day) {
          if (!breakPoint) {
            if (day.class === 'red') {
              if (selector === 1 && day.value > this.arrivalDay.value) { breakPoint = true } else if (selector > 1) breakPoint = true
    ...
    submitForm: function () {
      ...
      try {
        const inquiry = {
          ...
          fromDate:
            this.selectedDays[0].value +
            '. ' +
            this.currentMonth.name +
            ' ' +
            this.currentYear,
          toDate:
            this.selectedDays[this.selectedDays.length - 1].value +
            '. ' +
            this.currentMonth.name +
            ' ' +
            this.currentYear
        ...
        axios
          .post('https://www.h3.solutions/api/mailing/' + id, inquiry)
          .then(() => {
            this.successSubmit = true
          })
          .catch(() => {
            axios
              .post('https://www.h3.solutions/b-api/api/mailing' + id, inquiry)
              .then(() => {
                this.successSubmit = true
              })
              .catch((error) => {
