    ...
    <div v-if="!mobaDev" id="main-view">
      <div style="height: 100%; width: inherit; display: inherit">
        <div class="slide-wrap">
          <div
            ...
            class="btn-sm-g"
            style="left: 15px; top: 45%; right: unset"
          >
            <b-icon-chev-left style="color: #c9c9c9"></b-icon-chev-left>
          </div>
          <div
            style="
              width: 100%;
          ...
          >
            <div class="slider-wrapper">
              <div id="slider" class="slide-inner-wrapper">
                <div
                  v-for="(g, index) in sortGa"
                  v-bind:key="g.id"
                  class="img-container-g"
                  v-bind:style="{ width: ww + 'px' }"
                >
                  <div
                    class="fs-img-wrapper"
                    v-bind:style="{ width: g.width + 'px' }"
                  >
                    <img
                      id="img"
                      class="curr-img slide-img"
                      :src="g.src"
                      :alt="g.alt"
          ...
          <div
            ...
            @dblclick="preventDefault($event)"
            class="btn-sm-g"
            style="right: 15px; top: 45%"
          >
            <b-icon-chev-right style="color: #c9c9c9"></b-icon-chev-right>
          </div>
        </div>
      </div>
      <div id="bot-gallery-nav">
        <div style="position: relative; width: 100%; height: 100%">
              ...
              <div class="slide-inner-wrapper">
                ...
                >
                  <transition-group name="fadeIn" mode="out-in">
                    <div
                      v-for="(img, index) in sortGa"
                      v-bind:key="img.id"
                      @dblclick="preventDefault($event)"
                      @click="setViewImg(img, index)"
                      class="bn-parent"
                      v-bind:style="{
                        width: cw + 'px',
                        maxHeight: cw + 'px',
                    ...
                    >
                      <div
                        v-bind:class="[{ active: img.isActive }, 'bn-child']"
          ...
          <div
            class="position-absolute tl-0"
            style="width: 100%; margin-top: -45px"
          >
            <div class="filter-wrapper">
              <button class="f-main-btn goto-btn" @click="openFilter">
                Filter
              </button>
              <ul
                id="filter-content"
                class="filter-content flex-row flex-nowrap"
              >
                <li>
                  <div class="goto-btn">
                    <div
                      @click="sortGallery($event, 0)"
                      id="all"
                      class="btn btn-g widely activeSelector"
                    >
                ...
                <li>
                  <div class="goto-btn">
                    <div
                      @click="sortGallery($event, 1)"
                      id="outside"
                      class="btn btn-g widely"
                    >
                      <span style="text-transform: uppercase">{{
                        webData[1]
                      }}</span>
    ...
    <div v-else class="position-relative" style="padding-top: 100px">
      ...
      <div
        ...
        style="
          display: grid;
          overflow-y: scroll;
          box-shadow: 0 0 50px 64px #cecece inset;
        "
      >
    ...
    <transition name="fadeIn">
      <div v-if="toggleFullScreen" class="fs-container">
        <div class="fs-inner-container">
          <button @click="openFullScreen()" class="btn-sm-g inner-top">
            <b-icon-fs-exit></b-icon-fs-exit>
          </button>
          <div
          ...
          >
            <img
              :class="[mobaDev ? 'img-res' : 'curr-img']"
              :src="currImg.src"
              :alt="currImg.alt"
            />
          </div>
...
export default {
  ...
  methods: {
    ...
    setViewImg: function (img, index) {
      ...
      this.currImg = img
      this.currImg.isActive = true
      const to = this.slider.childNodes[index].offsetLeft
      this.slider.style.transform = 'translateX(-' + to + 'px)'
    },

    onImgLoad: function (e, index) {
      const w = document.getElementsByClassName('fs-img-wrapper')
      let img = null
      try {
        img = e.path[0]
      } catch {
      ...
      if (window.innerWidth > 768 && window.innerWidth < 1100) {
        this.gallery[index].width =
          (w[0].offsetHeight * img.naturalWidth) / img.naturalHeight / 2
      } else {
        this.gallery[index].width =
          (w[0].offsetHeight * img.naturalWidth) / img.naturalHeight
      }
      this.counter--
    },
    setChildWidth: function () {
      this.slider = document.querySelector('#slider')
      const p = document.getElementsByClassName('bn-parent')
      let h = null
      Array.prototype.forEach.call(p, (child) => {
        h = child.offsetHeight
      })
      this.cw = h
    },
    nextSlide: function () {
      const index = this.gallery.indexOf(this.currImg)
      const border = this.gallery.length - index
      if (border > 1 && this.mobaDev === false) {
        this.currImg.isActive = false
        const nextImg = this.gallery[index + 1]
        const to = this.slider.childNodes[index + 1].offsetLeft
        this.slider.style.transform = 'translateX(-' + to + 'px)'
    ...
    prevSlide: function () {
      ...
      if (border < this.gallery.length && this.mobaDev === false) {
        this.currImg.isActive = false
        const childImg = this.gallery[index - 1]
        const to = this.slider.childNodes[index - 1].offsetLeft
        this.slider.style.transform = 'translateX(-' + to + 'px)'
...
#bot-gallery-nav {
  ...
  bottom: 0;
  left: 0;
  border-top: 2px solid rgba(0, 105, 128, 0.625);
...
.bn-parent:hover .bn-child,
.bn-parent:focus .bn-child {
