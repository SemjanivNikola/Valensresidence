  ...
  <div id="nav">
    <div class="main-container" :style="{ width: ww + 'px' }">
      <div id="logo-icon" class="position-relative float-left">
        ...
        <img
          src="../assets/icons/stay_safe_in_croatia.png"
          alt="Stay safe in Croatia"
          class="res-img"
          style="width: 50%"
        />
      </div>
      ...
      <div
        id="menu-icon"
        class="float-right"
        style="height: 100px; width: 80px; display: inline-block"
      >
        <svg @click="openMenu()" viewBox="-10 -15 45 50">
          <g v-if="lcO" fill="#006980">
            <path
              class="svg-path-d"
              d="M4.5 11.5A.5.5 0 015 11h10a.5.5 0 010 1H5a.5.5 0 01-.5-.5zm-2-4"
            ></path>
            <path
              d="M2.5 7.5A.5.5 0 013 7h10a.5.5 0 010 1H3a.5.5 0 01-.5-.5zm-2-4"
            ></path>
            <path
              class="svg-path-u"
              d="M.5 3.5A.5.5 0 011 3h10a.5.5 0 010 1H1a.5.5 0 01-.5-.5z"
            ></path>
          </g>
          ...
          <div class="menu-destination">
            <ul>
              <transition-group name="slideDownMenu" mode="out-in">
                <li v-if="vLineAnimDone" v-bind:key="1">
                  <router-link to="/">{{ data[0] }}</router-link>
                </li>
                <li v-if="vLineAnimDone" v-bind:key="2">
                  <router-link to="main">{{ data[1] }}</router-link>
                </li>
                <li v-if="vLineAnimDone" v-bind:key="3">
                  <router-link to="gallery">{{ data[2] }}</router-link>
                </li>
                <li v-if="vLineAnimDone" v-bind:key="4">
                  <router-link to="contactUs">{{ data[3] }}</router-link>
                </li>
                <li v-if="vLineAnimDone" v-bind:key="5">
                  <a @click="setLang('eng')">ENG</a>
                </li>
                <li v-if="vLineAnimDone" v-bind:key="6">
                  <a @click="setLang('hr')">HR</a>
                </li>
              </transition-group>
            </ul>
          </div>
...
.slideDownMenu-enter-active,
.slideDownMenu-leave-active {
